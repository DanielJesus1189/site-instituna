...
import type { CreateFestivalInput } from '../types';
...
export function AddFestivalModal({ onClose, onSubmit, isSubmitting }: AddFestivalModalProps) {
  const [name, setName] = useState('');
  const [tunaName, setTunaName] = useState('');
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      setError('Não foi possível criar o festival. Tenta novamente.');
    }
  }
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4 p-5">
          ...
          <div className="mt-2 flex justify-end gap-2">
            ...
            >
              {isSubmitting ? 'A guardar…' : 'Guardar festival'}
            </button>
          </div>
        </form>
      </div>
